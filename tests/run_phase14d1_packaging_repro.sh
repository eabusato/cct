...
TMP_DIR="/tmp/cct_phase14d1_packaging"
...
make -C "$ROOT_DIR" dist DIST_DIR="$DIST_A" >/tmp/cct_phase14d1_dist_a.out 2>&1
make -C "$ROOT_DIR" dist DIST_DIR="$DIST_B" >/tmp/cct_phase14d1_dist_b.out 2>&1
...
if [ -x "$DIST_A/bin/cct" ]; then
  "$DIST_A/bin/cct" --help >/tmp/cct_phase14d1_help.out 2>&1
elif [ -f "$DIST_A/bin/cct.bat" ]; then
  "$DIST_A/bin/cct.bat" --help >/tmp/cct_phase14d1_help.out 2>&1
else
  echo "phase14d1: missing runnable cct wrapper in dist bundle" >&2
  exit 1
fi

if ! grep -q "Usage:" /tmp/cct_phase14d1_help.out; then
